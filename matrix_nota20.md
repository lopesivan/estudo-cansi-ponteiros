perfeito — aqui está a versão **C++20** usando `std::span` para iterar a matriz como um vetor linear, mantendo o mesmo layout “um `malloc` só” (tabela de linhas + dados contíguos + `free` automático via `unique_ptr`):
